...
class BackendRouteGeometry {
  ...
  /// Also tolerates nested `location` / GeoJSON from APIs.
  ...
  /// Terminals are prepended/appended when they are not already ~at the first/last stop
  /// (same rule as map pins), so the highlight connects the terminal to the next stop.
  ...
  /// Ordered labels + positions for UI: start terminal (if separate), each [route_stop], end terminal.
  static List<({String name, LatLng position})> labeledAnchorPointsFromDetail(
    Map<String, dynamic> data,
  ) {
    final rawStops = data['route_stops'] ?? data['routeStops'];
    final parsed = <({String name, LatLng p, int order, int index})>[];
    if (rawStops is List) {
      for (var i = 0; i < rawStops.length; i++) {
        final sm = _jsonObject(rawStops[i]);
        if (sm == null) continue;
        ...
        final label =
            nameRaw.isNotEmpty ? nameRaw : 'Bus stop ${parsed.length + 1}';
        parsed.add((
          name: label,
          p: LatLng(lat, lng),
          order: order,
    ...
    final se = startEndFromRouteDetail(data);
    final startLabel =
        _terminalDisplayName(data['start_terminal_id'] ?? data['startTerminalId']) ??
            'Start';
    final endLabel =
        _terminalDisplayName(data['end_terminal_id'] ?? data['endTerminalId']) ??
            'End';

    final ordered =
        parsed.map((e) => (name: e.name, position: e.p)).toList();

    if (ordered.isEmpty) {
      final out = <({String name, LatLng position})>[];
      if (se.start != null) {
        out.add((name: startLabel, position: se.start!));
      }
      if (se.end != null) {
        out.add((name: endLabel, position: se.end!));
      }
      return out;
    }
    ...
    final out = <({String name, LatLng position})>[];
    if (se.start != null &&
        !RouteStopDisplayUtils.nearTerminal(se.start!, ordered.first.position)) {
      out.add((name: startLabel, position: se.start!));
    }
    out.addAll(ordered);
    if (se.end != null &&
        !RouteStopDisplayUtils.nearTerminal(se.end!, out.last.position)) {
      out.add((name: endLabel, position: se.end!));
    }
    return out;
  }

  static String? _terminalDisplayName(dynamic v) {
    final m = _jsonObject(v);
    if (m == null) return null;
    final n = m['terminal_name'] ?? m['name'];
