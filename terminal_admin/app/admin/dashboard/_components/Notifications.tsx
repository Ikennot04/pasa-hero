  ...
  return (
    <div className="rounded-xl border border-base-300 bg-base-100 p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="badge badge-sm badge-outline">Last {sorted.length}</span>
      </div>
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-base-content/70 truncate">
                  {n.remarks ?? (n.auto_detected ? "Auto-detected" : "Manual update")}
                </p>
              </div>
              <div className="text-right shrink-0">
                <div className="text-sm font-semibold">{formatTime(n.event_time)}</div>
                <div className="text-xs text-base-content/60">{formatDate(n.event_time)}</div>
              </div>
            </div>
          ))
        ) : (
